...
Love Languages < SWIPE TYPE ≈ Enneagram < MBTI < Big Five
...
✅ Major Psychometric Advantages (Green Flags)
1. Normative Scoring (CRITICAL WIN)
Uses: 4-point Likert via swipes (Down/Left/Right/Up → 1/2/3/4)
...
Method: z_d = (mean_d - μ_person) / σ_person
...
Rule: If Δ(top1, top2) ≤ 0.5 SD → co-dominant
...
Concern: May not achieve α ≥ 0.80 with only 4-5 items
Target: Need 6-8 items minimum for reliability in most domains
Math check:
Expected reliability with k items (Spearman-Brown):
- 4 items, r̄=0.30: α = 0.63
- 6 items, r̄=0.30: α = 0.74  
- 8 items, r̄=0.30: α = 0.81

If average inter-item correlation is r̄=0.40:
- 4 items: α = 0.73
- 6 items: α = 0.83 ✓
Recommendation: Pilot test to verify if 4-5 item domains hit α ≥ 0.70. May need to add 1-2 items to weaker domains.

3. 8-Type Output from Axis Mapping
Method: Two composite axes → quadrants → 8 types
...
If yes → Enneagram layer is load-bearing and inherits its validation problems
 If no → It's decorative/interpretive → less concerning but potentially unnecessary
...
You measure:
✓ Relationships: Extensive (communication, time, affection, conflict, etc.)
✓ Some personality: Identity, energy, growth
✗ Missing: 
...
🚩 Red Flags (None Critical, But Monitor)
...
Domain reliability α ≥ 0.75 (most domains)
Test-retest r ≥ 0.75 (4-week interval)
...
Managing this:
✓ Your premium report emphasizes domains/channels (good)
✓ Free summary shows "top 2 channels" not just type (good)
⚠️ Monitor: Do users understand they might have different type on retest if near boundary?
...
For α = 0.80, need average r̄_item ≈ 0.45 (high)
...
Energy(3) - Very Risky
Growth(6) - Adequate
Domains with 3-4 items will struggle to achieve α ≥ 0.70 unless items are exceptionally well-correlated.
Recommendation:
Pilot with current 57 items
Calculate α for each domain
Add 2-3 items to domains with α < 0.70
Consider 65-70 items for final version
Alternative: Accept α = 0.65-0.70 for some domains, but report lower confidence for those areas.
...
Calculate Cronbach's α for all 12 domains
Calculate ω (omega) if you have multi-dimensional domains
Item-total correlations (flag items with r < 0.30)
Inter-item correlations within domains (should be r = 0.20-0.60)
Target: α ≥ 0.70 for all domains (0.80 ideal)
...
Domain scores (Pearson r, target ≥ 0.75)
Channel scores (Pearson r, target ≥ 0.75)
Type assignment (% agreement, target ≥ 75%)
...
- Positively with Big Five Extraversion (r ≈ 0.40-0.60)
- Positively with relationship satisfaction (r ≈ 0.30-0.50)

Your "Conflict" domain should correlate:
- Negatively with Big Five Neuroticism (r ≈ -0.30 to -0.50)
- With attachment anxiety (r ≈ 0.30-0.50)

Your channels should correlate modestly:
- With Love Languages scores (r ≈ 0.50-0.70)
...
In reports, show:
✓ Confidence level (high/medium/low) - you have this
✓ Delta values for close calls - you have this
✓ "Your scores" vs "typical range" context
✓ Uncertainty statements: "likely," "tends to," "often"

Avoid:
✗ Absolute statements: "You are..."
✗ Predictions without probability: "You will..."
✗ Diagnostic language: "Your conflict pattern indicates..."

5. Version Control & Iteration
Current: v0.1 (pre-validated)
→ Pilot: v0.2 (revised items, initial reliability data)
→ Beta: v0.3 (factor-analytic revision, norms)
→ Launch: v1.0 (validated, stability confirmed)
...
vs Love Languages:
✓ 12 domains vs. 5 (comprehensive)
✓ Conflict and boundaries (major relationship predictors)
✓ Normative scoring (statistical validity)
✓ Quality controls (flags, confidence)
✓ Behavior-focused items (more concrete)
vs Enneagram:
✓ Relationship-specific (not general personality)
✓ Clearer behavioral focus
✓ Shorter (57 vs 144 items)
✓ Mobile-optimized UX
✓ More sophisticated scoring
vs MBTI:
✓ Continuous scores maintained (not dichotomized)
✓ Mixed-type handling (co-dominance)
✓ Relationship focus (more relevant for your use case)
✓ No certification paywall
✓ Modern UX (swipe vs. dated forms)
vs Big Five:
✓ More engaging for lay users (types, not just numbers)
✓ Actionable relationship advice (not just trait descriptions)
✓ Shorter (57 vs. 240 items for NEO-PI-R)
✓ Domain-specific focus (relationships, not comprehensive)
...
Clinical diagnosis (relationship patterns ≠ pathology)
...
→ Tier A assessment
→ Best relationship-focused tool on market
→ Strong commercial + research potential
Moderate Case (25% probability):
Validation shows:
- Some domains weak (α = 0.60-0.70)
...
→ Tier B initially, iterate to Tier A
→ v2.0 with revised structure
→ Still viable commercial product
...
→ Tier C, needs major overhaul
→ Back to drawing board on item development
→ Unlikely given thoughtful specification, but possible
...
Assessment: PERFECTLY POSITIONED ✓
Your test sits exactly where you intended:
✓ More comprehensive than Love Languages (12 domains vs 5)
✓ Similar complexity to Enneagram (multi-faceted, growth-oriented)
✓ Less exhaustive than MBTI/Big Five (relationship-focused, not comprehensive)
✓ Strong focus on your stated areas (communication, growth, relationships)
...
✓ More comprehensive than Love Languages (single domain)
✓ Similar complexity to Enneagram (multi-faceted, growth-oriented)
✓ Less exhaustive than MBTI/Big Five (not covering all personality variance)
✓ Focus areas: Communication, relationships, growth, outlook, patterns

1. REALISTIC EXPECTATIONS FOR THIS TIER
What You SHOULD Expect:
✓ Adequate reliability (α = 0.70-0.85 per dimension)
Not gold standard, but acceptable for personal development use
Test-retest at 4-8 weeks should exceed 0.70
✓ Meaningful factor structure (even if not perfect)
...
✗ Comprehensive coverage of all personality domains
✗ Predictive validity for major life outcomes (jobs, health, longevity)
✗ Decades of peer-reviewed research
✗ Cross-cultural validation in 50+ countries
✗ Perfect psychometric properties (CFI > 0.95, α > 0.90)
...
Interpretation
r < 0.30
✓ Measuring something distinct (good!)
0.30 < r < 0.60
⚠️ Moderate overlap; has unique variance but also redundancy
r > 0.60
❌ Substantial redundancy; mostly relabeling Big Five
...
✓ Attachment theory: Clear developmental mechanism, specific relationship predictions
✓ Cognitive styles: Based on information processing research
✓ Emotional intelligence: Defined competencies with skill-building paths
Examples of incoherent frameworks:
✗ "We identified 12 archetypes through ancient wisdom" (no empirical basis)
✗ Arbitrary number of types (why 7? why not 6 or 8?)
✗ Circular definitions ("You're a Helper because you help people")
...
✓ Correlates with Big Five Extraversion: r = 0.55 (convergent)
✓ Correlates with Big Five Openness: r = 0.15 (discriminant)
✓ Correlates with self-rated directness: r = 0.62 (convergent)
✓ Correlates with math ability: r = 0.05 (discriminant)
...
🚩 1. No Reliability Data
If they can't provide Cronbach's α or test-retest correlations → STOP
This is basic psychometrics; absence means untested instrument
🚩 2. Ipsative Scores Used in Parametric Statistics
If they report:
...
→ Either they don't understand psychometrics or are misrepresenting findings
🚩 3. Perfect or Near-Perfect Reliability
If they claim α > 0.95 or test-retest r > 0.95:
...
→ Not a scientific instrument, a commercial product with no accountability
🚩 6. Unfalsifiable Complexity
Warning signs:
...
→ Adding complexity to avoid disconfirmation (Enneagram's weakness)

Yellow Flags (Concerning but Not Disqualifying):
⚠️ 1. Only Proprietary Validation
...
→ Transformation needed or population is skewed (could be real or measurement issue)
...
1. Calculate Cronbach's α
2. Item-total correlations (should be r > 0.30)
3. α if item deleted (should not increase by > 0.02)

If α < 0.70: Dimension is unreliable
If item-total r < 0.30: Item doesn't belong
If deleting item increases α substantially: Item is problematic
...
Continuous dimensions (anxiety × avoidance)
...
✓ Focused scope - Not trying to measure "all of personality" ✓ Clear mechanism - Explain HOW patterns develop and operate ✓ Behavioral anchors - Tied to observable actions, not just feelings ✓ Growth pathway - Show how to develop/change ✓ Falsifiable predictions - Can be proven wrong ✓ Incremental validity - Add something beyond Big Five
...
Criteria:
✓ Normative scoring OR ipsative with appropriate disclaimers
✓ Reliability α > 0.70 for all scales
✓ Test-retest > 0.70 at 1-2 months
✓ Some validity evidence (doesn't need to be extensive)
✓ Theoretical coherence
✓ Appropriate humility about claims
...
Criteria:
⚠️ Modest reliability (0.60 < α < 0.70)
⚠️ Limited validation data
⚠️ Proprietary with no independent verification
⚠️ Some theoretical coherence but gaps
⚠️ Makes modest claims
...
Tier C: Do Not Use
Criteria:
❌ Reliability data missing or insufficient (α < 0.60)
❌ No validity evidence
❌ Ipsative scores misused in statistics
❌ Unfalsifiable claims
❌ Refuses to share psychometric data
❌ Makes unsupported claims (predicts job success, relationship compatibility, etc.)
...
Red Flag Responses:
🚩 "Our proprietary algorithm is confidential" (transparency required)
🚩 "Reliability statistics don't apply to our holistic approach" (pseudoscience)
🚩 "Every major company uses us" (popularity ≠ validity)
🚩 "Research is ongoing/forthcoming" (should exist before marketing)
🚩 "You need our certification to interpret" (creates dependency, limits scrutiny)
🚩 "Results speak for themselves" (anecdote, not data)
...
Minimum acceptable: α > 0.70, test-retest > 0.70
...
Priced reasonably
→ This would be VALUABLE addition to the field
...
→ REJECT and recommend alternatives
...
If it has:
✓ Good user experience (engaging, clear, actionable)
✓ Adequate reliability (α > 0.70)
⚠️ Limited validity evidence
⚠️ Modest sample sizes
Consider:
...
Reliability data? (Must have α > 0.70)
...
Has some validity evidence (convergent/discriminant)?
   NO → Use with caution (Tier B)
   YES → Continue
   
Makes realistic claims consistent with evidence?
   NO → Use with caution (Tier B)
   YES → Likely acceptable (Tier A)
Bottom Line: For "Enneagram-depth" personal development tools, you don't need Big Five-level perfection, but you DO need basic psychometric competence. If they can show solid reliability, some validity evidence, and theoretical coherence, it's likely acceptable for your purposes—especially if it generates insights and engagement that more rigorous tools don't provide.
...
57-item Swipe Type assessment measures, and how we design questions, score responses, and generate reports. It’s written for (1) methodology reviewers, (2) item writers, and (3) scoring analysts. Scope (v0.1) Purpose: Surface stable relationship behavior patterns (“Swipe Types”) from quick, mobile
pasted
